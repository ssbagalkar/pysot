...
class SauronVideo(Video):
    ...
    def __init__(self, name, root, video_dir, init_rect, img_names, gt_rect, load_img=False):
        ##temp
        img_names = [os.path.join(root, name) for name in img_names]

        ## delete the above code later
        super(SauronVideo, self).__init__(name, root, video_dir,
...
class SauronDataset(Dataset):
    """
    Args:
        name: dataset name, should be 'UMICH2020'
        dataset_root: dataset root
        load_img: wether to load all imgs
    """
    def __init__(self, name, dataset_root, load_img=False):
        super(SauronDataset, self).__init__(name, dataset_root)
        ...
        for video in pbar:
            ...
            self.videos[video] = SauronVideo(video,
                                          dataset_root,
